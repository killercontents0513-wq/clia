"""Crawl A9LSLIM vacuum from LG Saudi Arabia."""
...
CODE = 'A9LSLIM'
URL  = 'https://www.lg.com/sa_en/vacuum-cleaners/a9lslim/'
...
name = name_m.group(1) if name_m else f'LG Vacuum {CODE}'
print(f'Name: {name}')
...
for p in all_paths_raw:
    ...
    if code_lower not in rel.lower() and 'a9l' not in rel.lower() and 'a9lslim' not in rel.lower(): continue
...
# Classify
gallery_paths = [p for p in unique if re.search(r'DZ-\d+|gallery|-\d{2}-\d{4}', p, re.I)
                 and 'thumbnail' not in p.lower() and 'mobilezoom' not in p.lower() and '-S-' not in p]
feat_paths    = [p for p in unique if p not in gallery_paths
                 and re.search(r'feature|desktop|-d\.jpg|-d-\d|-d-v', p, re.I)
                 and not re.search(r'mobile|-m\.jpg|-m-\d', p, re.I)]

print(f'\nGallery ({len(gallery_paths)}): {[p.split("/")[-1] for p in gallery_paths[:8]]}...')
print(f'Features ({len(feat_paths)}): {[p.split("/")[-1] for p in feat_paths[:8]]}...')

gallery = []
for p in gallery_paths[:16]:
    fn = p.split('/')[-1]
    alt = get_alt(combined, fn) or f'LG Vacuum {CODE}'
...
for p in feat_paths[:16]:
    ...
    t = re.sub(r'^[a-z0-9-]+-\d+-\d+-?', '', fn, flags=re.I)
    t = re.sub(r'-d(-\d+)?(-v\d+)?\.jpg$', '', t)
    t = re.sub(r'-d\.jpg$|-desktop\.jpg$', '', t)
    t = t.replace('-', ' ').strip().title()
    if not t or len(t) < 3:
        t = fn.replace('.jpg','').replace('-d','').replace('-',' ').title()
    d = ''
    idx = combined.find(t[:12]) if len(t) > 12 else -1
    if idx >= 0:
...
specs = {}
spec_keys = ['Suction','Battery','Runtime','Capacity','Weight','Noise','Filter','Accessory','Motor','Brush']
for k in spec_keys:
    idx = combined.find(k)
    if idx >= 0:
...
product = {
    ...
    'url': URL, 'cat': 'Vacuum', 'sub': 'Cordless', 'ico': 'vacuum', 'dv': 'HA',
    'gallery': gallery, 'features': features, 'specs': specs,
}
print(f'\nSummary: gal={len(gallery)}, feat={len(features)}, specs={len(specs)}')
with open(f'{BASE}/a9lslim_data.json', 'w', encoding='utf-8') as f:
    json.dump(product, f, ensure_ascii=False, indent=2)
print(f'Saved a9lslim_data.json')
